...
function activarDatos() {
    if (usuarioActivo[0]?.genero === "Female"){ 
        iHtml("h1", titulo, `¡Bienvenida ${usuarioActivo[0].nombre}!`)
    }else{
    ...
    iHtml('p', datosUsuario, `Genero: ${usuarioActivo[0]?.genero}`)
    iHtml('p', datosUsuario, `Profesión: ${usuarioActivo[0]?.trabajo}`)
}
